...
export default function SettingsPage() {
  ...
  if (loading || isLoading) {
    return (
      <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 py-24">
        <div className="text-stone-600 text-sm">Loading settings...</div>
      </div>
    );
  }
  ...
  if (roleInfo.role === "PROFESSIONAL" && roleInfo.professionType === "HARDWARE") {
    navItems.push({
      label: "Manage Shops",
  ...
  return (
    <DashboardShell navItems={navItems}>
      <header className="pb-6 border-b border-stone-200">
        <p className="text-orange-600 font-semibold tracking-widest uppercase text-sm mb-2">Settings</p>
        <h1 className="font-serif text-4xl text-stone-900">Profile Settings</h1>
        <p className="text-stone-500 mt-2">Manage account preferences and security settings.</p>
      </header>

      <div className="bg-white border border-stone-200 rounded-2xl p-8 space-y-4">
        <div>
          <div className="text-xs uppercase tracking-wider text-stone-400">Email</div>
          <div className="text-stone-700 font-semibold">{user?.email || "Not available"}</div>
        </div>
        <div className="text-sm text-stone-500">
          Password and notification settings will be added here next.
        </div>
      </div>
    </DashboardShell>
  );
